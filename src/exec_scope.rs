...
// instruction macros
macro_rules! peek {
    ($val:expr, $out:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        let index;
        match $val.val {
        ...
        set_var($out, &val, $global_scope, $stack, $cur_frame, $global_frame);
    }
}

macro_rules! call {
    ($func:expr, $scope:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        if func_exists($func, $scope, $global_scope) {
            let frame_func = get_func($func, $scope, $global_scope, $global_frame);
            let frame = frame_func.0;
            let func = frame_func.1;
            exec_func(&func, $global_scope, $stack, frame);
        } else {
...
macro_rules! add {
    ($a:expr, $b:expr, $out:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
...
macro_rules! mov {
    ($a:expr, $b:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        set_var($b, &$a.val, $global_scope, $stack, $cur_frame, $global_frame);
    }
}
...
macro_rules! not {
    ($a:expr, $out:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        let val = $a.val.not();
        set_var($out, &val, $global_scope, $stack, $cur_frame, $global_frame);
    };
}
...
macro_rules! get_type {
    ($typ:expr, $type_var:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $action:expr, $global_frame:expr) => {
        let type_var = get_var($type_var, $global_scope, $stack, $cur_frame, $global_frame);
...
macro_rules! get_name {
    ($name:expr, $name_var:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $action:expr, $global_frame:expr) => {
        let name_var = get_var($name_var, $global_scope, $stack, $cur_frame, $global_frame);
...
macro_rules! ref_ {
    ($index:expr, $out_var:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        // ugly line
        let out_var_type = get_var($out_var, $global_scope, $stack, $cur_frame, $global_frame).typ.typ[0].clone();
        match out_var_type {
            Types::POINTER => {
                set_var($out_var, &Values::POINTER($index, 1), $global_scope, $stack, $cur_frame, $global_frame);
            }
...
macro_rules! deref {
    ($ptr:expr, $out:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        let index;
        match $ptr.val {
...
macro_rules! pmov {
    ($val:expr, $ptr:expr, $offset:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        let ptr = get_var($ptr, $global_scope, $stack, $cur_frame, $global_frame);
...
macro_rules! alloc {
    ($typ:expr, $amnt:expr, $out:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
...
macro_rules! cmp {
    ($cond:expr, $a:expr, $b:expr, $out:expr, $global_scope:expr, $stack:expr, $cur_frame:expr, $global_frame:expr) => {
        {
            let c;
            match $cond.val {
            ...
            match c {
                0x00 => {
                    if $a.val == $b.val {
                        set_var($out, &Values::UNSIGNED(1), $global_scope, $stack, $cur_frame, $global_frame);
                    } else {
                        set_var($out, &Values::UNSIGNED(0), $global_scope, $stack, $cur_frame, $global_frame);
                    }
                }
                0x01 => {
                    if $a.val != $b.val {
...
pub fn exec_block(scope: &Scope, block: &Vec<Instruction>, global_scope: &Scope, stack: &mut Vec<Frame>, cur_frame: usize, pc: &mut usize, block_start: usize, global_frame: usize) -> i32 {
    ...
    while *pc - block_start < block.len() {
        ...
        match &instr.opcode {
            ...
            Opcode::PUSH_VAR(name) => { // PUSH [var]
                let var = get_var(name, global_scope, stack, cur_frame, global_frame);
            ...
            Opcode::POP(name) => { // POP [var]
                set_var(name, &stack[cur_frame].pop().val, global_scope, stack, cur_frame, global_frame);
            }

            Opcode::PEEK_IMM(val, out) => { // PEEK [imm] [var]
                peek!(val, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::PEEK_VAR(val_var, out) => { // PEEK [var] [var]
                let val = get_var(val_var, global_scope, stack, cur_frame, global_frame);

                peek!(val, out, global_scope, stack, cur_frame, global_frame);
            }

            Opcode::CALL_FUNC(func) => { // CALL [func]
                call!(func, scope, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::CALL_VAR(func_var) => { // CALL [var]
                let func_var = get_var(func_var, global_scope, stack, cur_frame, global_frame);
            ...
            Opcode::ADD_I_I(a, b, out) => { // ADD [imm] [imm] [var]
                add!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::ADD_V_I(a_name, b, out) => { // ADD [var] [imm] [var]
                let a = get_var(a_name, global_scope, stack, cur_frame, global_frame).clone();

                add!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::ADD_I_V(a, b_name, out) => { // ADD [imm] [var] [var]                
                let b = get_var(b_name, global_scope, stack, cur_frame, global_frame).clone();
            ...
            Opcode::SUB_I_I(a, b, out) => { // SUB [imm] [imm] [var]
                sub!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::MUL_I_I(a, b, out) => { // MUL [imm] [imm] [var]
                mul!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::DIV_I_I(a, b, out) => { // DIV [imm] [imm] [var]
                div!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::JMP_VAR(new_pc_name) => { // JMP [var]
                let new_pc_var = get_var(new_pc_name, global_scope, stack, cur_frame, global_frame).val.clone();
                let new_pc: usize;
            ...
            Opcode::JNE_I_I_V(a, b, c_name) => { // JNE [imm] [imm] [var]
                let c = get_var(c_name, global_scope, stack, cur_frame, global_frame).clone();
            ...
            Opcode::MOV_I_V(a, b) => { // MOV [imm] [var]
                mov!(a, b, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::MOV_VV_V(a_var, b) => { // MOV [var var] [var]
                let a_name;
                get_name!(a_name, a_var, global_scope, stack, cur_frame, "access", global_frame);
            ...
            Opcode::MOV_I_VV(a, b_var) => { // MOV [imm] [var var]
                let b;
                get_name!(b, b_var, global_scope, stack, cur_frame, "set", global_frame);
            ...
            Opcode::AND_I_I(a, b, out) => { // AND [imm] [imm]
                and!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::OR_I_I(a, b, out) => { // OR [imm] [imm]
                or!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::XOR_I_I(a, b, out) => { // XOR [imm] [imm]
                xor!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::NOT_IMM(a, out) => { // NOT [imm]
                not!(a, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::LSH_I_I(a, b, out) => { // LSH [imm] [imm]
                lsh!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::RSH_I_I(a, b, out) => { // RSH [imm] [imm]
                rsh!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::VAR_VAR_NAME(type_var, name) => { // VAR [var] [name]
                let typ;
                get_type!(typ, type_var, global_scope, stack, cur_frame, "create variable", global_frame);
                
                stack[cur_frame].create_var(name.clone(), typ);
            }
            Opcode::VAR_TYPE_VAR(typ, name_var) => { // VAR [type] [var]
                let name;
                get_name!(name, name_var, global_scope, stack, cur_frame, "create", global_frame);
            ...
            Opcode::RET_VAR(var) => { // RET [var]
                let v = get_var(var, global_scope, stack, cur_frame, global_frame).clone();
            ...
            Opcode::REF_IMM(val, out_var) => {
                ...
                ref_!(index, out_var, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::DEREF_IMM(ptr, out) => {
                deref!(ptr, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::DEREF_VAR(ptr_var, out) => {
                let ptr = get_var(ptr_var, global_scope, stack, cur_frame, global_frame);
            ...
            Opcode::INST_NAME(struct_name, out) => { // INST [name] [var]
                ...
                set_var(out, &strct, global_scope, stack, cur_frame, global_frame);
            ...
            Opcode::MOD_I_I(a, b, out) => { // MOD [imm] [imm] [var]
                modulo!(a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            ...
            Opcode::PMOV_IMM_IMM(val, ptr, offset) => {
                pmov!(val, ptr, offset, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::PMOV_VAR_IMM(val_var, ptr, offset) => {
                let val = get_var(val_var, global_scope, stack, cur_frame, global_frame).clone();

                pmov!(val, ptr, offset, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::PMOV_IMM_VAR(val, ptr, offset_var) => {
                let offset = get_var(offset_var, global_scope, stack, cur_frame, global_frame).clone();
            ...
            Opcode::ALLOC_TYPE_IMM(typ, amnt, out) => {
                alloc!(typ, amnt, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::ALLOC_VAR_IMM(type_var, amnt, out) => {
                let typ;
                get_type!(typ, type_var, global_scope, stack, cur_frame, "allocate", global_frame);

                alloc!(&typ, amnt, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::ALLOC_TYPE_VAR(typ, amnt_var, out) => {
                let amnt = get_var(amnt_var, global_scope, stack, cur_frame, global_frame);
            ...
            Opcode::FREE_VAR_IMM(ptr_var, amnt) => {
                let ptr = get_var(ptr_var, global_scope, stack, cur_frame, global_frame).clone();

                free_!(ptr, amnt, stack);
            }
            Opcode::FREE_IMM_VAR(ptr, amnt_var) => {
                let amnt = get_var(amnt_var, global_scope, stack, cur_frame, global_frame).clone();
            ...
            Opcode::CMP_I_I_I(cond, a, b, out) => {
                cmp!(cond, a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::CMP_V_I_I(cond_var, a, b, out) => {
                let cond = get_var(cond_var, global_scope, stack, cur_frame, global_frame).clone();

                cmp!(cond, a, b, out, global_scope, stack, cur_frame, global_frame);
            }
            Opcode::CMP_I_V_I(cond, a_var, b, out) => {
                let a = get_var(a_var, global_scope, stack, cur_frame, global_frame).clone();
            ...
            Opcode::CMP_I_I_V(cond, a, b_var, out) => {
                let b = get_var(b_var, global_scope, stack, cur_frame, global_frame).clone();
...
pub fn exec_scope(scope: &Scope, global_scope: &Scope, stack: &mut Vec<Frame>, cur_frame: usize, pop_stack: bool, pc: &mut usize, global_frame: usize) -> i32 {
    let scope_stack_start = stack[cur_frame].stack.len();

    for (_, module) in &scope.modules {
        let retval = exec_scope(&module.scope, &global_scope, stack, module.frame, false, &mut 0, module.frame);
        if retval != 0 {
            return retval;
        }
    }
    ...
    while i < scope.blocks.len() {
        ...
        let ret = match block {
            Block::CODE(vec) => exec_block(scope, vec, global_scope, stack, cur_frame, pc, start, global_frame),
            Block::SCOPE(scope) => {
                *pc += 1;
                exec_scope(&scope, global_scope, stack, cur_frame, pop_stack, &mut 0, global_frame)
            }
        };
...
pub fn exec_func(func: &Function, global_scope: &Scope, stack: &mut Vec<Frame>, global_frame: usize) -> i32 {
    ...
    let retval = exec_scope(&func.scope, global_scope, stack, len - 1, true, &mut 0, global_frame);
