...
pub enum Opcode { // very large enum
    ...
    MOV_IMM(Value)                    = 0x4A,
    MOV_VAR(String)                   = 0x4B,
    MOV_VARVAR(String)                = 0x4C,
    AND_I_I(Value, Value, String)     = 0x4D,
    AND_V_I(String, Value, String)    = 0x4E,
    AND_I_V(Value, String, String)    = 0x4F,
    AND_V_V(String, String, String)   = 0x50,
    OR_I_I(Value, Value, String)      = 0x51,
    OR_V_I(String, Value, String)     = 0x52,
    OR_I_V(Value, String, String)     = 0x53,
    OR_V_V(String, String, String)    = 0x54,
    XOR_I_I(Value, Value, String)     = 0x55,
    XOR_V_I(String, Value, String)    = 0x56,
    XOR_I_V(Value, String, String)    = 0x57,
    XOR_V_V(String, String, String)   = 0x58,
    NOT_IMM(Value)                    = 0x59,
    NOT_VAR(String)                   = 0x5A,
    LSH_I_I(Value, Value, String)     = 0x5B,
    LSH_V_I(String, Value, String)    = 0x5C,
    LSH_I_V(Value, String, String)    = 0x5D,
    LSH_V_V(String, String, String)   = 0x5E,
    RSH_I_I(Value, Value, String)     = 0x5F,
    RSH_V_I(String, Value, String)    = 0x60,
    RSH_I_V(Value, String, String)    = 0x61,
    RSH_V_V(String, String, String)   = 0x62,
    VAR_TYPE_NAME(Type, String)       = 0x63,
    VAR_VAR_NAME(String, String)      = 0x64,
    VAR_TYPE_VAR(Type, String)        = 0x65,
    VAR_VAR_VAR(String, String)       = 0x66,
    RET                               = 0x67,
    RET_IMM(Value)                    = 0x68,
    RET_VAR(String)                   = 0x69,
    DEREF_IMM(Value, String)          = 0x6A,
    DEREF_VAR(String, String)         = 0x6B,
...
impl Opcode {
    pub fn to_u8(self: &Opcode) -> u8 {
        match self {
            ...
            Opcode::MOV_VARVAR(_)       => 0x4C,
            Opcode::AND_I_I(_, _, _)    => 0x4D,
            Opcode::AND_V_I(_, _, _)    => 0x4E,
            Opcode::AND_I_V(_, _, _)    => 0x4F,
            Opcode::AND_V_V(_, _, _)    => 0x50,
            Opcode::OR_I_I(_, _, _)     => 0x51,
            Opcode::OR_V_I(_, _, _)     => 0x52,
            Opcode::OR_I_V(_, _, _)     => 0x53,
            Opcode::OR_V_V(_, _, _)     => 0x54,
            Opcode::XOR_I_I(_, _, _)    => 0x55,
            Opcode::XOR_V_I(_, _, _)    => 0x56,
            Opcode::XOR_I_V(_, _, _)    => 0x57,
            Opcode::XOR_V_V(_, _, _)    => 0x58,
            Opcode::NOT_IMM(_)          => 0x59,
            Opcode::NOT_VAR(_)          => 0x5A,
            Opcode::LSH_I_I(_, _, _)    => 0x5B,
            Opcode::LSH_V_I(_, _, _)    => 0x5C,
            Opcode::LSH_I_V(_, _, _)    => 0x5D,
            Opcode::LSH_V_V(_, _, _)    => 0x5E,
            Opcode::RSH_I_I(_, _, _)    => 0x5F,
            Opcode::RSH_V_I(_, _, _)    => 0x60,
            Opcode::RSH_I_V(_, _, _)    => 0x61,
            Opcode::RSH_V_V(_, _, _)    => 0x62,
            Opcode::VAR_TYPE_NAME(_, _) => 0x63,
            Opcode::VAR_VAR_NAME(_, _)  => 0x64,
            Opcode::VAR_TYPE_VAR(_, _)  => 0x65,
            Opcode::VAR_VAR_VAR(_, _)   => 0x66,
            Opcode::RET                 => 0x67,
            Opcode::RET_IMM(_)          => 0x68,
            Opcode::RET_VAR(_)          => 0x69,
            Opcode::DEREF_IMM(_, _)     => 0x6A,
            Opcode::DEREF_VAR(_, _)     => 0x6B,
            Opcode::REF(_, _)           => 0x6C,
